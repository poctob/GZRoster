...
public class DBObject {
	 ...
	 * @param dbman Database manager.
	 * @param table Table name.
	 * @param detail Hashmap with the properties
	 */
	public DBObject(DBManager dbman, String table, HashMap<String, String> details)
	{
		this.dbman=dbman;
		this.cols=details;
		this.table_name=table;
	...
	/**
	 * Convenience method to get values from the hashmap in the ArrayList format.
	 ...
	 * @param column Column that will be used in WHERE clause
	 ...
	 * @param column Column with a parameter.
	 * @param value Value of the parameter.
	 * @return sql statement string.
	 */
	protected String getSelect_sql(String column, String value)
	{
		String sql="SELECT * FROM "+this.table_name+" WHERE "+column+"='"+value+"'";
		return sql;
	}

	/**
	 * Generates Delete sql statement.
	 * @param column that contains delete parameter.
	 * @return sql statement string.
	 */
	private String getDelete_sql(String column)
	{
		String value=cols.get(column);
		String sql="DELETE FROM "+this.table_name+" WHERE "+column+"='"+value+"'";
		return sql;			
	}
	 ...
	 * @param column Column when value is expected.
	...
	/**
	 * Runs sql statement
	 * @param sql_str SQL statement to run
	 * @param wantresult Whether the results are expected back.
	 * @return ResultSet
	 */
	...
	{
		ResultSet rs=dbman.runQuery("SELECT NEXT VALUE FOR "+table_name+"_ID_GEN FROM RDB$DATABASE",true);
		try{
			if(rs.next())
			{
